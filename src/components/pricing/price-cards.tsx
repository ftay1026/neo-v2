...
export function PriceCards({ loading, frequency, priceMap }: Props) {
  const isSingleItem = PricingTier.length === 1;

  return (
    <div className={cn(
      "isolate mx-auto grid gap-8 lg:mx-0 lg:max-w-none"
      , isSingleItem ? 'grid-cols-1 justify-items-center' : 'grid-cols-1 lg:grid-cols-3'
    )}>
        ...
        <div key={tier.id} className={cn('rounded-lg bg-background/70 backdrop-blur-[6px] overflow-hidden')}>
          ...
          <div className={'px-8 mt-8'}>
            <Button className={'w-full'} variant={'secondary'} asChild={true}>
              <Link href={`/app/checkout/${tier.priceId[frequency.value]}`}>Pay now</Link>
            </Button>
          </div>
